...
public class ProductController {
    ...
    @PostMapping
    public ResponseEntity<ProductDTO> saveProduct(@RequestBody ProductRequestDTO productDTO) {
        ProductDTO response = productService.saveProduct(productDTO);
        return ResponseEntity.status(201).body(response); // 201 Created
    }

    @PutMapping(value = "/{id}")
    public ResponseEntity<ProductDTO> updateProduct(@PathVariable Long id, @RequestBody ProductRequestDTO productDTO) {
